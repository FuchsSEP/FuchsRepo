...
\begin{document}
	...
	\section{Package System}
		...
		\end{itemize}
		\subsection{Klasse Maintainance}
		
	\section{Package Util}
	\class{Mail}
	Die Klasse ist für die E-Mailbenachrichtigung der Benutzer zuständig. Sie ist sowohl für die automatisch gesendeten EMails, wie die Verifizierung oder Accountaktivierungsbestätigung, als auch für die von Kursleitern gesendeten Mails zuständig.
	\begin{itemize}
		\item \method{public boolean sentVerificationMail(int UserID)}
		\item \method{public boolean sendActivationMessage(int UserId)}
		\item \method{public boolean sendMail(ArrayList<String> mailRecipients, String sender, String subject, String message)}
	\end{itemize}
	\class{EncodingFilter}
	This class is used to implement the filter methods of the interface Filter to
	* enable UTF-8 encoding to prevent problems with character presentation.
	Die Klasse implementiert die Filtermethoden des Interface Filter um die UTF-8 Zeichenkodierung sicherzustellen und somit Probleme bei der Zeichendarstellung  zu vermeiden.
	\begin{itemize}
		\item \override
		...
		\item \method{protected String getEncoding(ServletRequest request)}
	\end{itemize}
	\class{HashPassword}
